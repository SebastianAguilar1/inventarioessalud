...
async function cleanAndSeed() {
  ...
  try {
    ...
    const { error: deleteError } = await supabase
      ...
      .delete()
      .neq('id', 0); // Eliminar todos excepto los que tienen id != 0 (que es ninguno)
    ...
    console.log('✅ Se insertaron correctamente 10 registros limpios y con UTF-8 correcto');
    console.log('\n📊 Resumen:');
    console.log(`   - 5 DESKTOP`);
    console.log(`   - 3 IMPRESORAS`);
    console.log(`   - 2 SCANNERS`);
    console.log(`\n✨ Los datos han sido reemplazados correctamente\n`);
    process.exit(0);
  } catch (err) {
